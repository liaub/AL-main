...
import pandas as pd  # original_data processing, CSV file I/O (e.g. pd.read_csv)
...
def preprocessingIncrementalDriftData(feature_length, DATA_FILE):
    ...
    file_dir = DATA_FILE +'/incremental'
    all_file_list = os.listdir(file_dir)
...
def LoadRealDriftData(DATA_FILE):
    all_file_list = os.listdir(DATA_FILE)
    train_data, train_original = None, None
    test_data, test_original = None, None
    for single_file in all_file_list:
        if 'mark' == single_file or 'point_statistic.pkl' == single_file:
            continue
        single_data_frame = pd.read_csv(os.path.join(DATA_FILE, single_file), sep=',', header=0)
        ...
        error_feature = (shift_data_1 / data).iloc[:, :-1]

        if 'test' in single_file:
            test_data = error_feature
            test_original = data
        else:
            train_data = error_feature
            train_original = data

    return train_data, train_original, test_data, test_original