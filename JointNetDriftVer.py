...
class Joint_PredictionVer(nn.Module):
    ...
    def forward(self,datax, BASE_PATH):
        #Get input original_data (center of mass)
        input = self.PrototypicalNet(datax)
        pre_type_y, pre_type_score = self.prototypical_loss(input, BASE_PATH)
        pre_loc_y = self.DriftPointNet(input, datax)
        # loc_acc = self.cal_loc_acc(pre_loc_y, locy)
        return pre_type_y, pre_type_score, pre_loc_y
    ...
    def prototypical_loss(self,input, BASE_PATH):
        '''
        ...
        return torch.argmax(log_p_y, -1), torch.max(torch.exp(log_p_y), -1).values
...
class DriftPointNet(nn.Module):
    ...
    def forward(self,centroid,datax):
        ...
        return torch.squeeze(pre_loc_y)
